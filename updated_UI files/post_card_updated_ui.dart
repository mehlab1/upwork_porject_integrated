...
class _PostCardState extends State<PostCard> {
  ...
  void _handleCommentError(Object error, StackTrace stackTrace) {
    final message = 'Comment thread update failed: $error';
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Something went wrong while posting your comment.'),
      ),
    );
  }
  ...
  List<CommentData> _cloneComments(List<CommentData> source) {
    ...
    return source
        .map(
          (comment) =>
              comment.copyWith(replies: _cloneComments(comment.replies)),
        )
        .toList();
  }
  ...
  void _addComment(String text) {
    ...
    try {
      ...
      final updated = [comment, ..._currentComments];
      setState(() {
        _comments = updated;
      });
  ...
  void _addReply(int parentIndex, String text) {
    final trimmed = text.trim();
    if (trimmed.length < 2) return;
    try {
      final sanitized = trimmed.replaceAll(RegExp(r'[^A-Za-z]'), '');
      ...
      if (parentIndex < 0 || parentIndex >= current.length) {
        throw RangeError.index(parentIndex, current);
      }
      final parent = current[parentIndex];
      final updatedReplies = List<CommentData>.from(parent.replies)..add(reply);
  ...
  Future<void> _showDeleteCommentDialog(
    ...
    if (result?.confirmed == true && context.mounted) {
      PalToast.show(context, message: 'Comment deleted successfully');
    }
  }
  ...
  Future<void> _showReportCommentSheet(
    ...
    if (result != null && context.mounted) {
      ...
      if (context.mounted) {
        PalToast.show(context, message: 'Report submitted successfully');
      }
    }
  }
}
