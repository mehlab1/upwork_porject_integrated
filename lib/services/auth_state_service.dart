...
class AuthStateService {
  ...
  Future<bool> shouldAutoLogin() async {
    try {
      ...
      final shouldAutoLogin = await _rememberMeService.shouldAutoLogin();
      
      if (!shouldAutoLogin) {
        // If Remember Me is false, clear any existing session
        // This ensures users who didn't check Remember Me are logged out
        final session = Supabase.instance.client.auth.currentSession;
        if (session != null) {
          // Clear the session if Remember Me is not enabled
      ...
      final session = Supabase.instance.client.auth.currentSession;
      if (session == null) {
        // No session, clear Remember Me preference
      ...
      if (expiresAt != null) {
        ...
        if (expiresAt < now) {
          // Session expired, clear Remember Me and sign out
          await _rememberMeService.clearRememberMe();
          await Supabase.instance.client.auth.signOut();
          return false;
        }
      ...
      // All checks passed, user should be auto-logged in
      return true;
    } catch (e) {
