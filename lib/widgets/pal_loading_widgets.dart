...
class _PalAnimatedLoadingBarState extends State<PalAnimatedLoadingBar>
  ...
  @override
  Widget build(BuildContext context) {
    const double containerWidth = 128;
          ...
          child: Stack(
            children: [
              AnimatedBuilder(
                animation: _controller,
                builder: (context, child) {
                  // Calculate position: starts from -barWidth (off left) to containerWidth (off right)
                  // This creates a wrap-around effect where the bar moves from left to right continuously
                  final double position = -barWidth + (_controller.value * (containerWidth + barWidth));
