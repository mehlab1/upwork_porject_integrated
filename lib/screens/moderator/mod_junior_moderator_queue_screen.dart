...
// class ModJuniorModeratorQueueScreen extends StatefulWidget {
//   const ModJuniorModeratorQueueScreen({super.key});

//   @override
//   State<ModJuniorModeratorQueueScreen> createState() =>
//       _ModJuniorModeratorQueueScreenState();
// }

// class _ModJuniorModeratorQueueScreenState
//     extends State<ModJuniorModeratorQueueScreen> {
...
//       children: [
//         const Text('NEW QUEUE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF62748E), letterSpacing: 0.6, fontFamily: 'Inter')),
//         const SizedBox(height: 12),
//         _buildPostCard(username: '@party_person', timeAgo: '5d ago', initials: 'PA', location: 'Surulere', topic: 'Ask', title: 'Surulere nightlife - Where\'s the party at?', body: 'Looking for good nightlife spots in Surulere. Not too expensive but with good music and vibes. What are your favorites? Clubs, bars, lounges?', voteCount: 87, commentCount: 1),
//         const SizedBox(height: 32),
//         const Text('HISTORY', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF62748E), letterSpacing: 0.6, fontFamily: 'Inter')),
//         const SizedBox(height: 12),
//         _buildPostCard(username: '@ikoyi_newbie', timeAgo: '4d ago', initials: 'IK', location: 'Ikoyi', topic: 'Ask', title: 'Best places to hang out in Ikoyi on weekends?', body: 'Just moved to Ikoyi and looking for cool spots to relax on weekends. Restaurants, lounges, parks - what do you recommend? Not trying to break the bank but willing to spend for quality.', voteCount: 142, commentCount: 1),
//         const SizedBox(height: 12),
//         _buildPostCard(username: '@party_person', timeAgo: '5d ago', initials: 'PA', location: 'Surulere', topic: 'Ask', title: 'Surulere nightlife - Where\'s the party at?', body: 'Looking for good nightlife spots in Surulere. Not too expensive but with good music and vibes. What are your favorites? Clubs, bars, lounges?', voteCount: 87, commentCount: 1),
//       ],
//     );
...
//         const Text('NEW QUEUE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF62748E), letterSpacing: 0.6, fontFamily: 'Inter')),
//         const SizedBox(height: 12),
//         _buildCommentCard(username: '@lagosian_boy', timeAgo: '1h ago', initials: 'LB', comment: 'You NEED to try the one at Yellow Chilli! Best I\'ve ever had, hands down.', voteCount: 45),
//         const SizedBox(height: 12),
//         _buildCommentCard(username: '@naija_gourmet', timeAgo: '30m ago', initials: 'NG', comment: 'Party jollof is undefeated! There\'s something about that smoky flavor from the firewood.', voteCount: 38),
//         const SizedBox(height: 32),
//         const Text('HISTORY', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Color(0xFF62748E), letterSpacing: 0.6, fontFamily: 'Inter')),
//         const SizedBox(height: 12),
//         _buildCommentCard(username: '@lagosian_boy', timeAgo: '1h ago', initials: 'LB', comment: 'You NEED to try the one at Yellow Chilli! Best I\'ve ever had, hands down.', voteCount: 45),
...
//         _buildCommentCard(username: '@naija_gourmet', timeAgo: '30m ago', initials: 'NG', comment: 'Party jollof is undefeated! There\'s something about that smoky flavor from the firewood.', voteCount: 38),
//         const SizedBox(height: 12),
//         _buildCommentCard(username: '@anonymous', timeAgo: 'just now', initials: 'AN', comment: 'checking', voteCount: 1),
//       ],
//     );
//   }
  ...
  Future<void> _fetchPosts(String queueType) async {
    if (queueType == 'new') {
      ...
      if (cached.isNotEmpty) {
        ...
        debugPrint('[JrModQueue] posts (new) from cache: ${posts.length}');
        _newPosts = posts;
        return;
      }
    } else {
      ...
      if (cached.isNotEmpty) {
        final posts = _extractList(cached, 'posts');
        debugPrint('[JrModQueue] posts (history) from cache: ${posts.length}');
        _historyPosts = posts;
        return;
      }
    }
    ...
    debugPrint('[JrModQueue] posts ($queueType) raw: ${response.data}');
    final posts = _extractList(response.data, 'posts');
    debugPrint('[JrModQueue] posts ($queueType) parsed count: ${posts.length}');
  ...
  Future<void> _fetchComments(String queueType) async {
    if (queueType == 'new') {
      ...
      if (cached.isNotEmpty) {
        ...
        debugPrint('[JrModQueue] comments (new) from cache: ${_newComments.length}');
        return;
      }
    } else {
      ...
      if (cached.isNotEmpty) {
        ...
        debugPrint('[JrModQueue] comments (history) from cache: ${_historyComments.length}');
        return;
      }
    }
    ...
    debugPrint('[JrModQueue] comments ($queueType) raw: ${response.data}');
    final comments = _extractList(response.data, 'comments');
    debugPrint('[JrModQueue] comments ($queueType) parsed count: ${comments.length}');
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child:
                        _buildTabButton('Comments', _selectedTab == 'Comments'),
                  ),
                ],
              ),
            ),
                      ...
                      children: [
                            ...
                            children: [
                              SvgPicture.asset(
                                ...
                                colorFilter: const ColorFilter.mode(
                                    Color(0xFF45556C), BlendMode.srcIn),
                              ),
                              const SizedBox(width: 6),
                              Text(
                                location,
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(8),
                            border: Border.all(color: const Color(0xFF7BF1A8), width: 0.756),
                          ),
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              SvgPicture.asset(
                                ...
                                height: 12,
                                colorFilter: const ColorFilter.mode(
                                    Color(0xFF008236), BlendMode.srcIn),
                              ),
                              const SizedBox(width: 6),
                              const Text(
                                'Ask',
          ...
          Row(
            children: [
              SvgPicture.asset(
                ...
                width: 16,
                height: 16,
                colorFilter: const ColorFilter.mode(Color(0xFF45556C), BlendMode.srcIn),
              ),
              const SizedBox(width: 8),
              Text(
              ...
              children: [
                Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Text('•', style: TextStyle(fontSize: 12, color: Color(0xFF90A1B9))),
                    const SizedBox(width: 8),
                    Text(
                      timeAgo,
                      style: const TextStyle(
