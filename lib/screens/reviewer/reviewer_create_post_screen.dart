...
class ReviewerCreatePostScreen extends StatefulWidget {
  const ReviewerCreatePostScreen({super.key, this.postData});

  final ReviewerPostCardData? postData;

  @override
  State<ReviewerCreatePostScreen> createState() => _JmCreatePostScreenState();
}
...
class _JmCreatePostScreenState extends State<ReviewerCreatePostScreen> {
  ...
  void _showGuidelinesDialog() {
    ...
    // The ReviewerCreatePostScreen state will be preserved when user returns
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => const ReviewerCommunityGuidelinesScreen(),
      ),
    );
  }
...
Future<bool?> showCreatePostModal(
  ...
  ReviewerPostCardData? postData,
}) {
  return showDialog<bool>(
    context: context,
    barrierDismissible: true,
    barrierColor: const Color(0xFF0B1120).withOpacity(0.55),
    builder: (context) => ReviewerCreatePostScreen(postData: postData),
  );
}
