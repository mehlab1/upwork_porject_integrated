...
class ReviewerContentCuratorQueueScreen extends StatefulWidget {
  const ReviewerContentCuratorQueueScreen({super.key});

  @override
  State<ReviewerContentCuratorQueueScreen> createState() =>
      _ReviewerContentCuratorQueueScreenState();
}

class _ReviewerContentCuratorQueueScreenState
    extends State<ReviewerContentCuratorQueueScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      if (cachedPosts.isNotEmpty && cachedComments.isNotEmpty) {
        ...
        debugPrint('[ReviewerCuratorQueue] from cache: ${posts.length} posts, ${comments.length} comments');
        if (mounted) {
          setState(() {
            _posts = posts;
            _comments = comments;
      ...
      debugPrint('[ReviewerCuratorQueue] posts raw=${postsResponse.data}');
      debugPrint('[ReviewerCuratorQueue] comments raw=${commentsResponse.data}');
      ...
      debugPrint('[ReviewerCuratorQueue] parsed ${_posts.length} posts, ${_comments.length} comments');
    } catch (e) {
      debugPrint('[ReviewerCuratorQueue] Error: $e');
      setState(() {
        _error = e.toString();
        _isLoading = false;
      });
