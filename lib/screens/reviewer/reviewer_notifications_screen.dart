...
class ReviewerNotificationsScreen extends StatefulWidget {
  const ReviewerNotificationsScreen({super.key});

  @override
  State<ReviewerNotificationsScreen> createState() => _JmNotificationsScreenState();
}

class _JmNotificationsScreenState extends State<ReviewerNotificationsScreen> {
  ...
  Future<void> _markAllNotificationsAsRead() async {
    try {
      ...
      debugPrint(
        '[ReviewerNotificationsScreen] Error marking all notifications as read: $e',
      );
    }
  }
  ...
  Future<void> _loadNotifications() async {
    ...
    try {
      ...
      debugPrint('[ReviewerNotificationsScreen] Error loading notifications: $e');
      if (!mounted) return;
      setState(() {
        _isPageLoading = false;
      });
                  ...
                  if (_unreadCount > 0)
                    Container(
                      constraints: BoxConstraints(
                        minWidth: _unreadCount >= 100 ? 36 : 28,
                        minHeight: 24,
                      ),
    ...
    if (item.avatarSvgAsset != null) {
      return Container(
        ...
        child: SvgPicture.asset(
          item.avatarSvgAsset!,
          width: item.avatarSvgSize ?? 24,
          height: item.avatarSvgSize ?? 24,
