...
import 'reviewer_delete_comment_dialog.dart';
import 'reviewer_delete_post_dialog.dart';
import 'reviewer_report_post_sheet.dart';
import 'reviewer_block_user_dialog.dart';
...
import '../reviewer_user_profile_screen.dart';
...
class ReviewerPostCardData {
  const ReviewerPostCardData({
  ...
  final List<ReviewerCommentData>? comments;

  final List<String>? badges;

  final String? userId;

  ReviewerPostCardData copyWith({int? votes, List<ReviewerCommentData>? comments}) {
...
class ReviewerCommentData {
  const ReviewerCommentData({
    required this.id,
  ...
  final List<ReviewerCommentData> replies;
  ...
  ReviewerCommentData copyWith({
    ...
    List<ReviewerCommentData>? replies,
...
class ReviewerPostCard extends StatefulWidget {
  const ReviewerPostCard({
    super.key,
    required this.data,
  ...
  final ReviewerPostCardData data;
  ...
  @override
  State<ReviewerPostCard> createState() => _JmPostCardState();
}

class _JmPostCardState extends State<ReviewerPostCard> {
  ...
  List<ReviewerCommentData> _comments = const [];
  ...
  ReviewerPostCardData get data => widget.data;

  List<ReviewerCommentData> get _currentComments {
  ...
  @override
  void didUpdateWidget(covariant ReviewerPostCard oldWidget) {
  ...
  /// Map comment from API response to ReviewerCommentData
  ReviewerCommentData? _mapCommentFromResponse(dynamic commentJson) {
    if (commentJson is! Map<String, dynamic>) return null;

    final id = commentJson['id']?.toString() ?? '';
    if (id.isEmpty) return null;

    final profile = commentJson['profiles'] as Map<String, dynamic>?;
    final username =
        (commentJson['username'] ?? profile?['username'] ?? '@user').toString();
    final author = username.startsWith('@') ? username : '@$username';

    final createdAt = commentJson['created_at'] != null
        ? DateTime.tryParse(commentJson['created_at'].toString())
        : null;

    final content = commentJson['content']?.toString() ?? '';
    final upvotes = _parseInt(commentJson['upvote_count'] ?? 0);
    final downvotes = _parseInt(commentJson['downvote_count'] ?? 0);
    final status = commentJson['status']?.toString() ?? 'active';

    // Extract user's vote state from API response
    int userVote = 0;
    final userVoteStr = commentJson['user_vote']?.toString().toLowerCase();
    ...
    final repliesData = commentJson['replies'] as List<dynamic>? ?? [];
    ...
    String? profilePictureUrl =
        (commentJson['profile_picture_url'] ??
                profile?['profile_picture_url'] ??
                commentJson['avatar_url'] ??
                profile?['avatar_url'])
            ?.toString();
  ...
  List<ReviewerCommentData> _cloneComments(List<ReviewerCommentData> source) {
    if (source.isEmpty) return <ReviewerCommentData>[];
  ...
  int _totalCommentCount(List<ReviewerCommentData> items) {
    int total = 0;
  ...
  Future<void> _addComment(String text) async {
    ...
    final optimisticComment = ReviewerCommentData(
      id: tempId,
    ...
    try {
      ...
      if (response['success'] == true &&
        ...
        setState(() {
          final updatedComments = List<ReviewerCommentData>.from(_currentComments);
  ...
  Future<void> _addReply(int parentIndex, String text) async {
    ...
    final current = List<ReviewerCommentData>.from(_currentComments);
    ...
    final optimisticReply = ReviewerCommentData(
      id: tempId,
    ...
    final updatedReplies = List<ReviewerCommentData>.from(parent.replies)
      ..add(optimisticReply);
      ...
      if (response['success'] == true &&
        ...
        setState(() {
          ...
          if (parentIndex < updatedComments.length) {
            ...
            final updatedParentReplies = List<ReviewerCommentData>.from(
  ...
  void _showAdminMenu(BuildContext context) {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _AdminPostMenu(
        ...
        onViewProfile: () {
          Navigator.of(context).pop();
          // Navigate to junior moderator user profile screen
          if (data.userId != null) {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => ReviewerUserProfileScreen(
                  userId: data.userId!,
  ...
  Widget _buildPostCardContainer({
    required ReviewerPostCardData data,
  ...
  Future<void> _showDeleteCommentDialog(
    ...
    ReviewerCommentData comment,
  ) async {
    final preview = comment.body.length > 60
        ? '${comment.body.substring(0, 57)}...'
        : comment.body;

    final result = await showDialog<ReviewerDeleteCommentResult>(
      context: context,

      barrierDismissible: false,

      builder: (_) => ReviewerDeleteCommentDialog(commentPreview: preview),
    );
    ...
    if (result?.confirmed == true && context.mounted) {
      try {
        ...
        setState(() {
          final updated = List<ReviewerCommentData>.from(_currentComments);
  ...
  void _removeCommentRecursive(List<ReviewerCommentData> comments, String commentId) {
    comments.removeWhere((comment) => comment.id == commentId);

    for (final comment in comments) {
      if (comment.replies.isNotEmpty) {
        final replies = List<ReviewerCommentData>.from(comment.replies);
  ...
  Future<void> _showReportPostSheet(BuildContext context) async {
    ...
    final result = await showModalBottomSheet<ReportResult?>(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => const ReviewerReportPostSheet(subject: ReportSubject.post),
    );
      ...
      try {
        ...
        if (context.mounted) {
          await showDialog<void>(
            context: context,
            barrierDismissible: false,
            builder: (_) => const ReviewerReportSuccessDialog(),
          );
  ...
  Future<void> _showDeleteDialog(BuildContext context, String title) async {
    ...
    final result = await showDialog<ReviewerDeletePostResult>(
      context: context,
      barrierDismissible: false,
      builder: (_) => ReviewerDeletePostDialog(postTitle: title),
    );
  ...
  Future<void> _handleBlockUser(BuildContext context, {String? userId}) async {
    ...
    final result = await showDialog<ReviewerBlockUserResult>(
      context: context,
      barrierDismissible: false,
      builder: (_) => ReviewerBlockUserDialog(username: username),
    );
  ...
  Future<void> _handleNominateWod(BuildContext context) async {
    ...
    try {
      if (data.userId == null || data.userId!.isEmpty) {
        ...
        PalToast.show(
          context,
          message: 'Cannot nominate due to user preferences',
          isError: true,
        );
        return;
      }

      final profileResponse = await _profileService.getProfileByUserId(data.userId!);
      final profile = profileResponse['profile'] as Map<String, dynamic>?;
      final dynamic optedRaw = profile?['wod_opted_in'];

      bool? wodOptedIn;
      if (optedRaw is bool) {
        wodOptedIn = optedRaw;
      } else if (optedRaw is num) {
        wodOptedIn = optedRaw != 0;
      } else if (optedRaw is String) {
        final normalized = optedRaw.trim().toLowerCase();
      ...
      final response = await _postService.nominateWod(postId: data.id!);
      ...
      final errorText = e.toString().replaceFirst('Exception: ', '').toLowerCase();
      final isPreferenceError =
          errorText.contains('wod_opted_in') ||
          errorText.contains('opted') ||
          errorText.contains('preference');
  ...
  Future<void> _showReportCommentSheet(
    ...
    final result = await showModalBottomSheet<ReportResult?>(
      ...
      builder: (_) => const ReviewerReportPostSheet(subject: ReportSubject.comment),
    );
...
Future<void> _showReportPostSheet(BuildContext context) async {
  final result = await showModalBottomSheet<ReportResult?>(
    ...
    builder: (_) => const ReviewerReportPostSheet(),
  );
...
class _CommentsSection extends StatelessWidget {
  ...
  final ValueChanged<ReviewerCommentData> onReportComment;

  final ValueChanged<ReviewerCommentData> onDeleteComment;

  final ValueChanged<ReviewerCommentData>? onBlockComment;
...
class _CommentCard extends StatefulWidget {
  ...
  final ReviewerCommentData comment;
...
class _AdminPostMenu extends StatelessWidget {
  ...
  final ReviewerPostCardData postData;
