...
class ReviewerFeedHomeScreen extends StatefulWidget {
  const ReviewerFeedHomeScreen({
    super.key, 
  ...
  @override
  State<ReviewerFeedHomeScreen> createState() => _JmFeedHomeScreenState();
}

class _JmFeedHomeScreenState extends State<ReviewerFeedHomeScreen> with AutomaticKeepAliveClientMixin {
  @override
  ...
  late final List<ReviewerPostCardData> _seedPosts = _buildSeedPosts().take(3).toList();
  final List<ReviewerPostCardData> _remotePosts = [];
  ...
  List<ReviewerPostCardData> _spotlightPosts = [];
  ...
  static const ReviewerPostCardData _pinnedAdminPost = ReviewerPostCardData(
  ...
  List<ReviewerPostCardData> get _allPosts => [..._seedPosts, ..._remotePosts];

  List<ReviewerPostCardData> get _filteredPosts => _postsForFilter(_selectedFilter);

  List<ReviewerPostCardData> get _visiblePosts {
    // If showing spotlight posts, return those instead
    if (_isShowingSpotlightPosts) {
      if (_spotlightPosts.isEmpty) return const <ReviewerPostCardData>[];
    ...
    if (posts.isEmpty) return const <ReviewerPostCardData>[];
    final limit = math.min(_visiblePostLimit, posts.length);
    return posts.take(limit).toList();
  }

  List<ReviewerPostCardData> _postsForFilter(String filter) {
    switch (filter) {
      case 'Hot':
  ...
  Future<void> _fetchSpotlightPosts({bool reset = false}) async {
    ...
    try {
      ...
      final mappedPosts = postsList
          ...
          .whereType<ReviewerPostCardData>()
          .toList();
  ...
  Future<void> _fetchProfilesForPosts(List<Map<String, dynamic>> posts) async {
    ...
    for (final post in posts) {
      final userId = _extractUserId(post);
  ...
  @override
  void didUpdateWidget(covariant ReviewerFeedHomeScreen oldWidget) {
  ...
  List<ReviewerPostCardData> _buildSeedPosts() {
    return [
      ReviewerPostCardData(
  ...
  Future<void> _fetchFeed({bool reset = false, bool forceRefresh = false}) async {
    ...
    try {
      ...
      final mappedPosts = <ReviewerPostCardData>[];
  ...
  ReviewerPostCardData? _mapPostToCardData(
    ...
    return ReviewerPostCardData(
      ...
      votes: votes,
      // Don't set avatarAsset for dynamic posts - let ReviewerPostCard use profilePictureUrl/initials
  ...
  String? _extractUserId(Map<String, dynamic> post) {
    ...
    final profileMap = post['profiles'];
    if (profileMap is Map<String, dynamic>) {
      final nestedProfileId = normalize(profileMap['id'] ?? profileMap['user_id']);
      if (nestedProfileId != null) return nestedProfileId;
    }

    final userMap = post['user'];
    if (userMap is Map<String, dynamic>) {
      final nestedUserId = normalize(userMap['id'] ?? userMap['user_id']);
        ...
        children: [
          ...
          if (_isInitialPostsLoading || (_isFeedFetching && _remotePosts.isEmpty && !_isLoadingSpotlightPosts))
            ...
            if (_selectedFilter == 'New' && !_isShowingSpotlightPosts) ...[
              ReviewerPostCard(
                data: const ReviewerPostCardData(
              ...
              const SizedBox(height: 24),
              ReviewerPostCard(data: _pinnedAdminPost, isPinnedAdmin: true),
              const SizedBox(height: 24),
            ],
              ...
              ...postsToShow.map((post) {
                try {
                  return Padding(
                    ...
                    child: ReviewerPostCard(data: post),
                  );
                } catch (e, stackTrace) {
