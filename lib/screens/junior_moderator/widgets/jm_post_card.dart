...
import 'jm_delete_comment_dialog.dart';
import 'jm_delete_post_dialog.dart';
import 'jm_report_post_sheet.dart';
import '../../../widgets/report_connection_error_dialog.dart';
import 'jm_block_user_dialog.dart';
...
import '../jm_user_profile_screen.dart';
...
class JmPostCardData {
  const JmPostCardData({
  ...
  final List<JmCommentData>? comments;
  ...
  JmPostCardData copyWith({int? votes, List<JmCommentData>? comments}) {
...
class JmCommentData {
  ...
  final List<JmCommentData> replies;
...
class JmPostCard extends StatefulWidget {
  const JmPostCard({
    super.key,
    required this.data,
  ...
  @override
  State<JmPostCard> createState() => _JmPostCardState();
}

class _JmPostCardState extends State<JmPostCard> {
  ...
  List<JmCommentData> _comments = const [];
  ...
  JmPostCardData get data => widget.data;

  List<JmCommentData> get _currentComments {
  ...
  @override
  void didUpdateWidget(covariant JmPostCard oldWidget) {
  ...
  /// Map comment from API response to JmCommentData
  JmCommentData? _mapCommentFromResponse(dynamic commentJson) {
  ...
  List<JmCommentData> _cloneComments(List<JmCommentData> source) {
    if (source.isEmpty) return <JmCommentData>[];
  ...
  int _totalCommentCount(List<JmCommentData> items) {
    int total = 0;
  ...
  Future<void> _addComment(String text) async {
    ...
    final optimisticComment = JmCommentData(
      id: tempId,
    ...
    try {
      ...
      if (response['success'] == true &&
        ...
        setState(() {
          final updatedComments = List<JmCommentData>.from(_currentComments);
  ...
  Future<void> _addReply(int parentIndex, String text) async {
    ...
    final current = List<JmCommentData>.from(_currentComments);
    ...
    final optimisticReply = JmCommentData(
      id: tempId,
    ...
    final updatedReplies = List<JmCommentData>.from(parent.replies)
      ..add(optimisticReply);
      ...
      if (response['success'] == true &&
        ...
        setState(() {
          ...
          if (parentIndex < updatedComments.length) {
            ...
            final updatedParentReplies = List<JmCommentData>.from(
        ...
        onViewProfile: () {
          ...
          if (data.userId != null) {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (context) => JmUserProfileScreen(
                  userId: data.userId!,
  ...
  Future<void> _showDeleteCommentDialog(
    ...
    JmCommentData comment,
  ) async {
    final preview = comment.body.length > 60
        ? '${comment.body.substring(0, 57)}...'
        : comment.body;

    final result = await showDialog<JmDeleteCommentResult>(
      context: context,

      barrierDismissible: false,

      builder: (_) => JmDeleteCommentDialog(commentPreview: preview),
    );
    ...
    if (result?.confirmed == true && context.mounted) {
      try {
        ...
        setState(() {
          final updated = List<JmCommentData>.from(_currentComments);
  ...
  void _removeCommentRecursive(List<JmCommentData> comments, String commentId) {
    comments.removeWhere((comment) => comment.id == commentId);

    for (final comment in comments) {
      if (comment.replies.isNotEmpty) {
        final replies = List<JmCommentData>.from(comment.replies);
  ...
  Future<void> _showReportPostSheet(BuildContext context) async {
    ...
    final result = await showModalBottomSheet<ReportResult?>(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => const JmReportPostSheet(subject: ReportSubject.post),
    );
      ...
      try {
        ...
        if (context.mounted) {
          await showDialog<void>(
            context: context,
            barrierDismissible: false,
            builder: (_) => const JmReportSuccessDialog(),
          );
          if (context.mounted) {
            PalToast.show(
              context,
  ...
  Future<void> _showDeleteDialog(BuildContext context, String title) async {
    ...
    final result = await showDialog<JmDeletePostResult>(
      context: context,
      barrierDismissible: false,
      builder: (_) => JmDeletePostDialog(postTitle: title),
    );
  ...
  Future<void> _handleBlockUser(BuildContext context, {String? userId}) async {
    ...
    final result = await showDialog<JmBlockUserResult>(
      context: context,
      barrierDismissible: false,
      builder: (_) => JmBlockUserDialog(username: username),
    );
  ...
  Future<void> _showReportCommentSheet(
    ...
    final result = await showModalBottomSheet<ReportResult?>(
      ...
      builder: (_) => const JmReportPostSheet(subject: ReportSubject.comment),
    );
...
Future<void> showReportPostSheet(BuildContext context) async {
  final result = await showModalBottomSheet<ReportResult?>(
    context: context,
    ...
    builder: (_) => const JmReportPostSheet(),
  );
...
Future<void> showDeleteDialog(BuildContext context, String title) async {
  final result = await showDialog<JmDeletePostResult>(
    context: context,
...
class _CommentsSection extends StatelessWidget {
  ...
  final ValueChanged<JmCommentData> onReportComment;

  final ValueChanged<JmCommentData> onDeleteComment;

  final ValueChanged<JmCommentData>? onBlockComment;
