...
class JmCreatePostScreen extends StatefulWidget {
  const JmCreatePostScreen({super.key, this.postData});

  final JmPostCardData? postData;

  @override
  State<JmCreatePostScreen> createState() => _JmCreatePostScreenState();
}

class _JmCreatePostScreenState extends State<JmCreatePostScreen> {
  ...
  void _showGuidelinesDialog() {
    ...
    // The JmCreatePostScreen state will be preserved when user returns
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => const JmCommunityGuidelinesScreen(),
      ),
    );
  }
...
Future<bool?> showCreatePostModal(
  ...
  JmPostCardData? postData,
}) {
  return showDialog<bool>(
    context: context,
    barrierDismissible: true,
    barrierColor: const Color(0xFF0B1120).withOpacity(0.55),
    builder: (context) => JmCreatePostScreen(postData: postData),
  );
}
